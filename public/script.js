  ...
  if (formEl && inputEl && resultEl) {
    formEl.addEventListener('submit', async (e) => {
      ...
      if (!staffId) {
        ...
        resultEl.textContent = 'Please enter your Staff ID.';
        return;
      }
      ...
      try {
        const resp = await api('/staff/role', {
          method: 'POST',
        ...
        resultEl.innerHTML = `
          <div class="mb8">Authentication complete</div>
          <div><b>Name:</b> ${data.name || '—'}</div>
          <div><b>Staff ID:</b> ${staffId}</div>
          <div><b>Role:</b> <span class="pill">${data.role}</span></div>
        `;
      } catch (err) {
